...
def load_data(root, batch_size, num_workers):
    ...
    logger.info('query_dataset length:{}'.format(len(query_dataset)))
    logger.info('train_dataset length:{}'.format(len(train_dataset)))
    logger.info('retrieval_dataset length:{}'.format(len(retrieval_dataset)))
...
class Food101(Dataset):
    ...
    @staticmethod
    def init(root):
        image_class_labels = pd.read_csv(os.path.join(root, 'labels.txt'), names=['target'])
        ...
        images_train = pd.read_csv(os.path.join(root,'train.txt'), names=['filepath'])
        images_test = pd.read_csv(os.path.join(root,'test.txt'), names=['filepath'])
        train_images = []
        test_images = []
